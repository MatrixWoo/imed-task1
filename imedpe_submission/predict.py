"""
Participant entry point for the iMED Pose Estimation challenge.

CONTRACT
--------
Input  (read-only mount at /input):
    /input/<sequence_name>/
        K.txt                                  intrinsics for all 4 cameras
        endoscope1/L/frame_*.png               left  view, scope 1
        endoscope1/R/frame_*.png               right view, scope 1
        endoscope2/L/frame_*.png               left  view, scope 2
        endoscope2/R/frame_*.png               right view, scope 2

Output (writable mount at /output):
    /output/<sequence_name>/pose_predictions.txt
        One row per frame, whitespace-separated:
            <frame_idx> <tx> <ty> <tz> <qx> <qy> <qz> <qw>
        Frame 0 must be (t=0, q=identity); subsequent frames are world-from-camera
        in the coordinate frame anchored at frame 0 of endoscope2/L.
        Use NaN for any frame you cannot register.
        Translation is scale-ambiguous (scoring uses Sim(3) Umeyama alignment).
        Quaternion convention: (qx, qy, qz, qw) - scipy default.

Runtime constraints:
    - 1x RTX 4090, CUDA 11.8 host driver
    - 10 minutes total wall-clock for all sequences
    - --network=none (no internet at runtime; bake weights into the image)
    - 20 GB RAM
    - Read-only /input, writable /output, nothing else persists

REPLACE the body of `predict_sequence` with your method.
"""
from __future__ import annotations

import argparse
import os
import sys
import time
from pathlib import Path


# ----------------------------------------------------------------------------
# Replace the body of this function with your method.
# ----------------------------------------------------------------------------
def predict_sequence(sequence_dir: Path, output_dir: Path) -> None:
    """
    Read images from `sequence_dir` and write a `pose_predictions.txt` file
    inside `output_dir / sequence_dir.name`.

    See the contract docstring at the top of this file.
    """
    # --- example stub: write the identity trajectory --------------------------
    e2l_dir = sequence_dir / "endoscope2" / "L"
    frame_paths = sorted(e2l_dir.glob("frame_*.png"))
    frame_ids = [int(p.stem.split("_")[1]) for p in frame_paths]

    out_path = output_dir / sequence_dir.name / "pose_predictions.txt"
    out_path.parent.mkdir(parents=True, exist_ok=True)

    lines = []
    for fid in frame_ids:
        # Identity pose for every frame -- replace this!
        lines.append(
            f"{fid} 0.0000000000 0.0000000000 0.0000000000 "
            f"0.0000000000 0.0000000000 0.0000000000 1.0000000000"
        )
    out_path.write_text("\n".join(lines) + "\n")
    # --- end stub -------------------------------------------------------------


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--input-dir", type=Path,
                        default=Path(os.environ.get("INPUT_DIR", "/input")))
    parser.add_argument("--output-dir", type=Path,
                        default=Path(os.environ.get("OUTPUT_DIR", "/output")))
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    sequence_dirs = sorted(p for p in args.input_dir.iterdir() if p.is_dir())
    if not sequence_dirs:
        print(f"ERROR: No sequence directories under {args.input_dir}", file=sys.stderr)
        sys.exit(2)

    print(f"Found {len(sequence_dirs)} sequence(s).", flush=True)
    t0 = time.perf_counter()
    for seq in sequence_dirs:
        seq_t0 = time.perf_counter()
        predict_sequence(seq, args.output_dir)
        print(f"  {seq.name}: {time.perf_counter() - seq_t0:.2f}s", flush=True)
    print(f"Total: {time.perf_counter() - t0:.2f}s", flush=True)


if __name__ == "__main__":
    main()